...
func (r *ChatRepo) CreateSession(ctx context.Context, session *model.ChatSession) error {
	_, err := r.db.Exec(ctx, `INSERT INTO chat_sessions
		(id, user_id, subject_id, title, llm_provider, llm_model, created_at, updated_at)
		VALUES ($1, $2, NULLIF($3, '')::uuid, $4, $5, $6, $7, $7)`,
		session.ID, session.UserID, session.SubjectID, session.Title,
...
func (r *ChatRepo) SaveTurn(ctx context.Context, session *model.ChatSession, message *model.ChatMessage) error {
	...
	_, err = tx.Exec(ctx, `INSERT INTO chat_messages
		(id, session_id, user_id, role, content, answer, citations, metadata, created_at)
		VALUES ($1, $2, $3, 'turn', $4, $5, $6::jsonb, $7::jsonb, $8)
		...
		message.ID, message.SessionID, message.UserID, message.Question, message.Answer,
