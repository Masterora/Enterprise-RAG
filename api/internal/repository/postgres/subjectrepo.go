...
func (r *SubjectRepo) Create(ctx context.Context, subject *model.Subject) error {
	_, err := r.db.Exec(
		ctx,
		`INSERT INTO subjects (id, tenant_id, name, description, owner_id, visibility, created_at, updated_at)
...
func (r *SubjectRepo) GetAccessibleByID(ctx context.Context, subjectID, userID, tenantID string) (*model.Subject, error) {
	var (
	...
	err := r.db.QueryRow(
		ctx,
		`SELECT id::text, tenant_id::text, name, description, owner_id::text, visibility, created_at, updated_at
		 FROM subjects
		 WHERE id = $1
		   AND tenant_id = $3
...
func (r *SubjectRepo) ListAccessible(ctx context.Context, filter model.SubjectListFilter) ([]model.Subject, int64, error) {
	conditions := []string{"deleted_at IS NULL", "tenant_id = $1", "(owner_id = $2 OR visibility = 'public')"}
	...
	rows, err := r.db.Query(
		ctx,
		fmt.Sprintf(`SELECT id::text, tenant_id::text, name, description, owner_id::text, visibility, created_at, updated_at
			FROM subjects
			WHERE %s
...
func (r *SubjectRepo) ExistsAccessible(ctx context.Context, subjectID, userID, tenantID string) (bool, error) {
	var exists bool
	err := r.db.QueryRow(
		ctx,
		`SELECT EXISTS (
...
func (r *SubjectRepo) UpdateByOwner(ctx context.Context, subject *model.Subject) error {
	tag, err := r.db.Exec(
		...
		`UPDATE subjects
		 ...
		 WHERE id = $1 AND owner_id = $2 AND tenant_id = $7 AND deleted_at IS NULL`,
		subject.ID,
		subject.OwnerID,
		subject.Name,
		sql.NullString{String: subject.Description, Valid: subject.Description != ""},
...
func (r *SubjectRepo) SoftDeleteByOwner(ctx context.Context, subjectID, userID, tenantID string) (bool, error) {
	...
	if _, err := tx.Exec(
		...
		`UPDATE documents
		 SET deleted_at = now(), updated_at = now()
		 WHERE subject_id = $1 AND user_id = $2 AND tenant_id = $3 AND deleted_at IS NULL`,
		subjectID,
		userID,
		tenantID,
	); err != nil {
