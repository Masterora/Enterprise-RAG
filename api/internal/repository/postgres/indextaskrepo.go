...
func (r *IndexTaskRepo) List(ctx context.Context, filter model.IndexTaskListFilter) ([]model.IndexTask, int64, error) {
	conditions := []string{"t.tenant_id = $1", "t.user_id = $2", "t.cleared_at IS NULL"}
...
func (r *IndexTaskRepo) UpdateStatus(ctx context.Context, taskID, status, errMsg string) error {
	...
	result, err := r.db.Exec(
		ctx,
		`UPDATE index_tasks
		 SET status = $2, error_message = $3, lease_expires_at = NULL, updated_at = now()
...
func (r *IndexTaskRepo) Claim(ctx context.Context, taskID string, lease time.Duration) (bool, error) {
	...
	result, err := r.db.Exec(
		...
		`UPDATE index_tasks
		 ...
		 WHERE id = $1
		   AND (
		     status = $4
		     OR (status = $2 AND COALESCE(lease_expires_at, '-infinity'::timestamptz) <= now())
		   )`,
		taskID,
...
func (r *IndexTaskRepo) ScheduleRetry(ctx context.Context, taskID string, maxRetries int) (*model.IndexTask, bool, error) {
	...
	err := r.db.QueryRow(
		ctx,
		`UPDATE index_tasks
		 SET status = $2, retry_count = retry_count + 1, lease_expires_at = NULL, updated_at = now()
...
func (r *IndexTaskRepo) Retry(ctx context.Context, taskID, userID string) (*model.IndexTask, error) {
	...
	err = tx.QueryRow(
		ctx,
		`UPDATE index_tasks
		 SET status = $3, retry_count = retry_count + 1, error_message = NULL, lease_expires_at = NULL, updated_at = now()
		 WHERE id = $1 AND user_id = $2 AND status = $4
		 RETURNING id::text, tenant_id::text, doc_id::text, subject_id::text, user_id::text,
		           task_type, status, retry_count, error_message, COALESCE(metadata, '{}'::jsonb),
	...
	documentStatus := map[string]string{
		model.TaskTypeParse:     model.DocumentStatusUploaded,
		model.TaskTypeChunk:     model.DocumentStatusParsed,
		model.TaskTypeEmbedding: model.DocumentStatusChunked,
		model.TaskTypeDelete:    model.DocumentStatusDeleting,
	}[task.TaskType]
	previous, ok := model.DocumentRetryPreviousStatuses(documentStatus)
	if !ok {
		return nil, ErrIndexTaskNotRetryable
	}
	result, err := tx.Exec(ctx, `UPDATE documents SET status = $2, error_message = NULL, updated_at = now()
		WHERE id = $1 AND user_id = $3 AND deleted_at IS NULL AND status = ANY($4)`,
		task.DocID, documentStatus, userID, previous)
	if err != nil {
		return nil, err
	}
	if result.RowsAffected() == 0 {
	...
	if err := enqueueOutbox(ctx, tx, &task, true); err != nil {
		return nil, err
	}
