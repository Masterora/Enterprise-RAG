import { cancelChatRun, streamChat, type AgentStep } from '../../api/chat'
  ...
  try {
    await streamChat(
	      ...
	      },
	      {
	          onRun: (value) => {
	            runID = value
	            input.callbacks.updateMessage(input.sessionID, input.messageID, { runID: value })
	          },
          ...
          onDone: (answer) => {
            completed = true
            const insufficient = answered === false || isNoAnswer(answer)
            ...
            input.callbacks.updateMessage(input.sessionID, input.messageID, {
              status: insufficient ? input.labels.insufficient : input.labels.done,
              ...(answer ? { answer } : {}),
              errorReason: '',
              finishedAt: Date.now(),
              loading: false,
    ...
    if (runID) {
      await cancelChatRun(runID).catch(() => false)
    }
    throw error
  } finally {
    window.clearTimeout(timeout)
...
function isNoAnswer(answer: string) {
  ...
  return ['无法确定', '无法回答', '资料不足，无法确定'].includes(normalized)
}
...
const chatStatusKeys: Record<string, string> = {
  ...
  'agent.citation.validate': 'chat.status.processing',
