...
export function DocumentsPage() {
  ...
  async function handleUpload(values: { subject_id: string; file?: UploadFile[] }) {
    ...
    const oversizedFile = originalFiles.find((file) => file.size > maxUploadFileBytes)
    if (oversizedFile) {
      message.error(t('documents.fileTooLarge', { name: oversizedFile.name }))
      return
    }
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleUpload}>
          ...
          <Form.Item
            ...
            name="file"
            extra={t('documents.uploadLimitHint')}
            ...
            <Upload
              beforeUpload={(file) => {
                if (file.size > maxUploadFileBytes) {
                  message.error(t('documents.fileTooLarge', { name: file.name }))
